...
public final class VictorPhysiologyTest {
    ...
    @Test public void lowRiskGetsLease(){VictorPhysiology.Runtime r=runtime();VictorPhysiology.ActionProposal p=safe();VictorPhysiology.GateDecision d=r.evaluate(p);assertEquals(VictorPhysiology.DecisionStatus.AUTHORIZED,d.status);assertNotNull(d.lease);assertTrue(d.lease.validFor(p));}
