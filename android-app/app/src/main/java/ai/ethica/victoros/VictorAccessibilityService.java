...
public final class VictorAccessibilityService extends AccessibilityService {
    ...
    @Override public void onAccessibilityEvent(AccessibilityEvent event){
        if(event==null)return;long now=System.currentTimeMillis();if(now-lastPublishMs<250)return;lastPublishMs=now;
        ...
        List<CharSequence> eventText=event.getText();if(eventText!=null&&!eventText.isEmpty())payload.append("eventText=").append(eventText).append('\n');
    ...
    @Override public void onInterrupt(){GodsEyeWorldModel.publishText(getApplicationContext(),"ACTIVE_UI","accessibility interrupted","active UI sensor interrupted");}
}
