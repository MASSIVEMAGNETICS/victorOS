...
public final class GodsEyeWorldModel {
    ...
    public static synchronized Observation publishText(Context context,String source,String text,String summary){
        ...
        String payload=text==null?"":text,hash=VictorStore.sha256(payload);LATEST_TEXT.put(source,payload);return commit(store,source,summary==null?"text observation":summary,hash,payload.getBytes(java.nio.charset.StandardCharsets.UTF_8).length);
    }
    public static synchronized Observation publishBinary(Context context,String source,byte[] bytes,String summary){
        VictorStore store=new VictorStore(context.getApplicationContext());if(store.load().humanStop)return blocked(source,"HUMAN STOP");if(!store.isLedgerTrusted())return blocked(source,"LEDGER CORRUPT");
        byte[] input=bytes==null?new byte[0]:bytes,bounded=input.length<=MAX_BINARY_BYTES?Arrays.copyOf(input,input.length):Arrays.copyOf(input,MAX_BINARY_BYTES);String hash=VictorStore.sha256(input);LATEST_BINARY.put(source,bounded);return commit(store,source,summary==null?"binary observation":summary,hash,input.length);
    }
