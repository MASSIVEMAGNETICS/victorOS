...
public final class GodsEyeWorldModel {
    ...
    public static synchronized Observation publishText(Context context,String source,String text,String summary){
        ...
        String hash=VictorStore.sha256(payload);
        LATEST_TEXT.put(source,payload);
        return commit(context,source,summary==null?"text observation":summary,hash,payload.getBytes(java.nio.charset.StandardCharsets.UTF_8).length);
    }
    ...
    public static synchronized Observation publishBinary(Context context,String source,byte[] bytes,String summary){
        ...
        byte[] bounded=input.length<=MAX_BINARY_BYTES?Arrays.copyOf(input,input.length):Arrays.copyOf(input,MAX_BINARY_BYTES);
        String hash=VictorStore.sha256(input);
        LATEST_BINARY.put(source,bounded);
        return commit(context,source,summary==null?"binary observation":summary,hash,input.length);
    }

    private static Observation commit(Context context,String source,String summary,String hash,int bytes){
        Observation o=new Observation(source,summary,hash,bytes);
        RECENT.addFirst(o); while(RECENT.size()>MAX_RECENT)RECENT.removeLast();
        new VictorStore(context.getApplicationContext()).recordGodsEyeMetadata(source,hash,bytes,summary);
        return o;
    }
    ...
    public static synchronized String summary(){
        if(RECENT.isEmpty())return "No live observations in this process.";
        StringBuilder out=new StringBuilder();int n=0;for(Observation o:RECENT){out.append(o).append('\n');if(++n>=12)break;}return out.toString().trim();
    }
}
