...
public final class VictorMicrophoneService extends Service {
    ...
    private void startAudio(){
        ...
        recorder.startRecording();running=true;worker=new Thread(()->{short[] samples=new short[4096];long last=0;
            while(running){if(new VictorStore(getApplicationContext()).load().humanStop){stopSelf();break;}int n=recorder.read(samples,0,samples.length);if(n<=0)continue;long now=System.currentTimeMillis();if(now-last<500)continue;last=now;
