...
public final class VictorMicrophoneService extends Service {
    ...
    @Override public int onStartCommand(Intent intent,int flags,int startId){
        ...
        try{startAudio();return START_NOT_STICKY;}catch(Exception e){GodsEyeWorldModel.publishText(getApplicationContext(),"MIC_ERROR",e.getClass().getSimpleName()+": "+e.getMessage(),"microphone start error");stopSelf();return START_NOT_STICKY;}
    }
    private void startAudio(){
        int sampleRate=16000;int min=AudioRecord.getMinBufferSize(sampleRate,AudioFormat.CHANNEL_IN_MONO,AudioFormat.ENCODING_PCM_16BIT);if(min<=0)throw new IllegalStateException("AudioRecord buffer unavailable");
        recorder=new AudioRecord(MediaRecorder.AudioSource.VOICE_RECOGNITION,sampleRate,AudioFormat.CHANNEL_IN_MONO,AudioFormat.ENCODING_PCM_16BIT,Math.max(min,sampleRate*2));if(recorder.getState()!=AudioRecord.STATE_INITIALIZED)throw new IllegalStateException("AudioRecord init failed");
        recorder.startRecording();running=true;worker=new Thread(()->{short[] samples=new short[4096];long last=0;
            while(running){VictorStore store=new VictorStore(getApplicationContext());if(store.load().humanStop||!store.isLedgerTrusted()){stopSelf();break;}int n=recorder.read(samples,0,samples.length);if(n<=0)continue;long now=System.currentTimeMillis();if(now-last<500)continue;last=now;
